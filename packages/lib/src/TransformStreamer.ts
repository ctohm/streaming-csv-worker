...
import { createEmptyResult, FetchPapaStreamer } from './FetchPapaStreamer';
...
export class TransformStreamer extends TransformStream<Uint8Array, Uint8Array> {
	...
	constructor(config: Partial<PapaConfig> = {} as Partial<PapaConfig>) {
		...
		const textencoder = new TextEncoder(),
			...
			emitter = new CSVStreamersEmitter(),
			transformContent = {
	...
	parseChunk(chunk: string): ParseResult<unknown> {
		...
		if (
			/**
			 * Store completeResults only if partial results arent already handled 
		...
		if (!this._completed
			...
			if (isFunction(this._config.complete)) this._config.complete(this._completeResults);
